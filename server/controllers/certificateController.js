...
const getCertificateDataForStudent = async (studentId, req) => {
  ...
  // APPROVAL & COMPLETION CONDITIONS
  const teacherApproved = certificate.teacherApproved === true;
  const managerApproved = certificate.managerApproved === true;
  const isInternshipCompleted = (internship.status || "").toLowerCase() === "completed";
  
  // Certificate is strictly approved only when Teacher + Manager approved AND status is completed
  const certificateApproved = teacherApproved && managerApproved && isInternshipCompleted;
  ...
  return {
    ...
    internship: {
      ...
      rejectedAt: internship.rejectedAt || null,
    },
    ...
    companyName:
      certificate.companyName || internship.companyName || "Company Name",
    internshipStartDate:
      certificate.internshipStartDate || internship.startDate || null,
    internshipEndDate:
      certificate.internshipEndDate || internship.endDate || null,
...
const getMyCertificateStatus = async (req, res) => {
  try {
    ...
    const data = await getCertificateDataForStudent(student._id, req);
...
const getStudentCertificate = async (req, res) => {
  try {
    ...
    const data = await getCertificateDataForStudent(studentId, req);
...
const verifyCertificate = async (req, res) => {
  try {
    ...
    // Check internship status for public verification
    const studentId = certificate.student || certificate.certificateId;
    const internship = await Internship.findOne({ student: studentId }).lean();
    const isCompleted = (internship?.status || "").toLowerCase() === "completed";

    const verified = teacherApproved && managerApproved && isCompleted;

    if (!verified) {
      return res.status(200).json({
        success: true,
        verified: false,
        message: "Certificate found, but it has not been fully approved or completed yet.",
      });
    }
