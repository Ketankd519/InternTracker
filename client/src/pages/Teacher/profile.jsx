...
export default function TeacherProfile() {
  ...
  try {
    ...
    data.append("mobileNo",formData.mobileNo.trim());
    ...
    // CREATE PROFILE
    if (!profileSaved) {
      response = await API.post("/teacher/profile",data);
    }

    // UPDATE PROFILE
    else {
      response = await API.put("/teacher/profile",data);
    }
    ...
    setFormData((previous) => ({
      ...
      mobileNo: teacher.mobileNo !== undefined &&
      ...
      // IMPORTANT
      signature: teacher.signature || previous.signature,
    }));
    ...
    const errorMessage = err.response?.data?.message || "Unable to save teacher profile.";
          ...
          <div className="teacher-signature-instructions">
            <strong>Signature Guidelines:</strong>
              <div className="signature-sample-image">
                <img src="/images/signature-guidelines.png"
