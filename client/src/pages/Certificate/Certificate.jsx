...
export default function Certificate() {
  ...
  const fetchCertificate = async () => {
    try {
      ...
      const isCompleted =
        (data?.internshipStatus || data?.internship?.status || "").toLowerCase() === "completed";

      // QR Code only generates when teacher + manager approved AND internship status is completed
      if (
      ...
      setError(
        err.response?.data?.message || "Unable to fetch certificate information."
      );
    } finally {
      setLoading(false);
    }
  ...
  // STATUS & APPROVAL CONDITIONS
  const teacherApproved = data.teacherApproved === true;
  const managerApproved = data.managerApproved === true;
  const internshipStatus =
    (data.internshipStatus || data.internship?.status || "").toLowerCase();
  const isInternshipCompleted = internshipStatus === "completed";

  // LOCKED / UNLOCKED: All 3 must be true
  const certificateApproved =
    teacherApproved && managerApproved && isInternshipCompleted;
    ...
    <div className="final-certificate-page">
      ...
      <div
        ...
        className={`final-certificate ${
          certificateApproved ? "certificate-unlocked" : "certificate-locked"
        }`}
      >
        {/* BACKGROUND TEMPLATE */}
        <img
        ...
        {/* STUDENT NAME */}
        <div className="certificate-student-name">{studentName}</div>

        {/* COMPANY NAME */}
        <div className="certificate-company-name">{companyName}</div>

        {/* START DATE */}
        <div className="certificate-start-date">{formatDate(startDate)}</div>

        {/* END DATE */}
        <div className="certificate-end-date">{formatDate(endDate)}</div>

        {/* TEACHER APPROVAL */}
        {teacherApproved && <div className="teacher-approved-mark"> ✓ </div>}

        {/* MANAGER APPROVAL */}
        {managerApproved && <div className="manager-approved-mark"> ✓ </div>}

        {/* TEACHER NAME */}
        <div
          className={`certificate-teacher-name ${
            teacherVerified ? "visible" : "hidden"
          }`}
        >
          Teacher: {teacherName}
        </div>
        ...
        {/* MANAGER NAME */}
        <div
          className={`certificate-manager-name ${
            managerVerified ? "visible" : "hidden"
          }`}
        >
          Manager: {managerName}
        </div>
      ...
      <div className="certificate-download-section">
        {!certificateApproved && (
          <p className="certificate-download-disabled-message">
            {!isInternshipCompleted
              ? "Certificate will unlock after your internship status is marked as Completed."
              : "Certificate download will be available after both Teacher and Manager approvals."}
          </p>
        )}

        {certificateApproved && (
          <p className="certificate-download-ready-message">
            ✓ Certificate verified, completed, and approved by Teacher and Manager.
          </p>
        )}
        ...
        <button
          ...
          className={`certificate-download-btn ${
            certificateApproved ? "enabled" : "disabled"
          }`}
